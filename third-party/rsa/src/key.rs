...
impl RsaPrivateKey {
    ...
    ///
    /// Even when called without an explicit blinding RNG, the underlying
    /// private-key operation is blinded with the OS CSPRNG — see the talkrypt
    /// patch in `algorithms/rsa.rs` and `TALKRYPT-PATCH.md` (RUSTSEC-2023-0071).
    ...
    ///
    /// As with [`Self::decrypt`], the underlying private-key operation is blinded
    /// with the OS CSPRNG even without an explicit RNG — see the talkrypt patch
    /// in `algorithms/rsa.rs` and `TALKRYPT-PATCH.md` (RUSTSEC-2023-0071).
