...
pub fn rsa_decrypt<R: CryptoRngCore + ?Sized>(
    ...
    // Type annotation needed because, with the unblinded `Cow::Borrowed` arm now
    // behind `#[cfg(not(getrandom))]`, both live arms are `Cow::Owned` (talkrypt
    // patch) and the borrowed type can no longer be inferred from the arms.
    let c: Cow<'_, BigUint> = if let Some(ref mut rng) = rng {
        let (blinded, unblinder) = blind(rng, priv_key, c);
        ir = Some(unblinder);
        Cow::Owned(blinded)
    } else {
        // talkrypt patch (RUSTSEC-2023-0071, "Marvin"): this is the single
        // chokepoint for every RSA private-key operation (decrypt AND sign, via
        // rsa_decrypt_and_check). Upstream runs it UNBLINDED when no caller RNG
        // was supplied — the variable-time private-key op the Marvin attack uses
        // to recover the key. We instead blind with the OS CSPRNG, so a private
        // key operation is NEVER performed unblinded. The unblinded fallback
        // remains only for no-`getrandom` builds, which talkrypt never produces.
        // See TALKRYPT-PATCH.md.
        #[cfg(feature = "getrandom")]
        {
            let (blinded, unblinder) = blind(&mut rand_core::OsRng, priv_key, c);
            ir = Some(unblinder);
            Cow::Owned(blinded)
        }
