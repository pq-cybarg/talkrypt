...
| F-10 | Info | GUI bundles (Android APK, desktop) are not built/tested in CI; the Rust core + FFI they depend on are. | Partially addressed — the **Android** module now has a CI compile + JVM-unit gate (`.github/workflows/android.yml`): it regenerates the uniffi Kotlin bindings from the current FFI, compiles the whole app, and runs the JVM unit tests on any `android/**` or `crates/ffi/**` change — catching FFI-drift (non-exhaustive `when`, missing callback overrides, API mismatches) that previously merged silently. Residual: it is a compile+unit gate, not an instrumented/device test (no NDK `.so`, no emulator run), and the **desktop** GUI bundle still has no dedicated GUI job (its logic rides the workspace `cargo test`). |
